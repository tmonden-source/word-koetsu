// _evidence-core.js — 証拠抽出の共通ロジック（プロンプト生成・API呼び出し・整形）
// evidence-process-background.js から利用する。

export function buildPrompts(complaint, side) {
  const MAX = 40000;
  const target = String(complaint || "").slice(0, MAX);
...
"3. 本文だけでは確定できない項目（標目の正確な名称、作成者、作成日、原本写しの別）は、" +
"推定値を入れたうえで note に『要確認』と明記する。推測が全くできない項目は空文字にし note に理由を書く。\n" +
"4. 立証趣旨は本文の主張と証拠の対応関係から作成してよいが、事実を創作しないこと。\n" +
"5. 標目・作成者等は実際の証拠物を見て確定すべきものであり、本下書きはあくまで仮であることを前提とする。\n\n" +
"【重要】証拠として引用されていないものを創作してはならない。本文に出てくる証拠だけを対象とする。\n\n" +
"【事件情報の抽出】本文の冒頭・末尾等から、次を抽出する：\n" +
...
'"evidences":[{"number":"甲1または乙1（実際の区分）","title":"","originalOrCopy":"写し","date":"","author":"","purpose":"","note":""}],"notes":["全体的な注意があれば"]}';
  ...
  const userContent =
"次の書面本文から、証拠説明書の下書きをJSONで作成してください。\n\n――― 本文 ここから ―――\n" +
target + "\n――― 本文 ここまで ―――";
...
// Claude API を呼び、整形済みの結果オブジェクトを返す（高精度のSonnetを使用）
export async function runEvidenceExtraction(complaint, side, apiKey) {
  const { systemPrompt, userContent } = buildPrompts(complaint, side);
  ...
  if (!anthropicRes.ok) {
    ...
    throw new Error("解析APIの呼び出しに失敗しました: " + detail.slice(0, 300));
  }
  ...
  const evList = Array.isArray(parsed.evidences) ? parsed.evidences : [];
  evList.forEach((e) => {
