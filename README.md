...
### 構成ファイル
- `netlify/functions/evidence-start.js` … 証拠抽出ジョブの受付（受付番号を即返す）
- `netlify/functions/evidence-process-background.js` … バックグラウンドでSonnetが抽出（最大15分・時間切れ回避）
- `netlify/functions/evidence-result.js` … 受付番号で結果を取得（ポーリング）
- `netlify/functions/_evidence-core.js` … 抽出の共通ロジック
- 結果の一時保存に Netlify Blobs（`@netlify/blobs`）を使用
...
委任契約書（法人破産）のひな形を開いてボタンを押すと、パネルに金額・支払方法・署名情報を入力して契約書に差し込みます。AI・サーバーは使わず、アドイン内で計算・差込みが完結します。

### 機能
- 法人破産費用・事務手数料・預り金を入力すると、合計とうち預り金を自動計算して金額表に反映
- 支払方法：一括／分割を選択。分割は合計額を対象に、初回一括額（任意）を引いた残りを指定回数で月割り（端数は最終回）、1円/1000円単位の丸めに対応
- 支払期間・毎月額・最終回額を自動で文面化
- 署名欄：住所・会社名・代表者名を「会社名　代表者氏名」の形で記載

### 構成ファイル
- `src/contract.html` / `src/contract.js` … タスクパネル
- `src/manifest-contract.xml` … Word用マニフェスト（GUIDは他アドインと別）

### 注意
- ひな形が旧形式（.doc）の場合は、先に .docx 形式で保存してから使用してください。
- 金額・分割計算は機械的に行うため正確ですが、最終的な契約内容は必ず確認してください。
